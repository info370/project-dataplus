...
yelp.data <- fromJSON(json_file) %>% filter(!is.na(price)) %>% filter(!is.na(reviewCount)) %>% filter(!is.na(censusMedianHHIncome)) %>% filter(!is.na(censusIncomePerCapita)) %>% filter(!is.na(censusGiniIndexOfInequality)) %>% filter(!is.na(rating)) %>% filter(!is.na(category)) %>% filter(!is.na(censusTract)) %>% filter(!is.na(healthInspectionScore))
...
yelp.data$healthInspectionResult <- as.factor(yelp.data$healthInspectionResult)
yelp.data$healthViolationType <- as.factor(yelp.data$healthViolationType)

# create dummy variables for healthInspectionResult and healthViolationType

dmy <- dummyVars(~ healthInspectionResult + healthViolationType, data=yelp.data) # notice how we specify which columns to "dumbify""
...
# only using healthInspectionResult dummy variables: Satisfactory and unsatisfactory
above.avg.wo.census <- subset(above.avg, select=c("reviewCount", "rating", "healthInspectionScore", "healthViolationPoints", "healthInspectionGrade", "healthViolationType.","healthViolationType.blue","healthViolationType.red"))
below.avg.wo.census <- subset(below.avg, select=c("reviewCount","rating", "healthInspectionScore", "healthViolationPoints", "healthInspectionGrade", "healthViolationType.","healthViolationType.blue","healthViolationType.red"))

check.unique.dummy.variable <- function(dataset) {
  result.complete <-  length(unique(dataset$healthInspectionResult.Complete)) > 1
  result.incomplete <- length(unique(dataset$healthInspectionResult.Incomplete)) > 1
  result.not.ready <- length(unique(dataset$healthInspectionResult.Not.Ready.For.Inspection)) > 1
  result.satisfactory <- length(unique(dataset$healthInspectionResult.Satisfactory)) > 1
  result.unsatisfactory <- length(unique(dataset$healthInspectionResult.Unsatisfactory)) > 1
  result.violationtype <- length(unique(dataset$healthViolationType.)) > 1
  result.violationBlue <- length(unique(dataset$healthViolationType.blue)) > 1
  result.violationRed <- length(unique(dataset$healthViolationType.red)) > 1
  return(c(result.complete,result.incomplete,result.not.ready,result.satisfactory,result.unsatisfactory,result.violationtype,result.violationBlue,result.violationRed))
}
